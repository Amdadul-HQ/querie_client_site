...
const QueryPage = () => {
  ...
  useEffect(() => {
    axios
      .get(
        `https://query-rouge.vercel.app/queryallpost?page=${currentPage}&size=${itemPerPage}&search=${search}`
      )
      .then((res) => {
        setPosts(
          res.data.sort(
  ...
  useEffect(() => {
    axios
      .get(`https://query-rouge.vercel.app/queryCount`)
      .then((res) => {
  ...
  const pages = [...Array(Math.ceil(parseInt(count) / itemPerPage)).keys()].map(
    (element) => element + 1
  );
    ...
    <section className="container mx-auto font-poppins">
      ...
      <div className="flex gap-x-1 lg:gap-x-3 justify-center">
        <form
          ...
          className="flex items-center md:flex-row flex-col justify-center gap-x-1 lg:gap-x-4"
        >
          <div>
          ...
          <div className="md:flex-row md:mt-0 mt-4 flex gap-x-2 ">
            <div className="flex">
              <button
                type="submit"
                className="text-xl text-black hover:text-white hover:bg-black border-2 border-black transition duration-300 lg:px-5 px-2 py-2 rounded-lg"
              >
                Search
              </button>
            </div>
      ...
      <div
        className={`grid grid-cols-1 gap-8 mt-8 md:mt-16 md:grid-cols-2 ${
          changeLayout ? "xl:grid-cols-3" : "xl:grid-cols-2"
        }`}
      >
        {posts &&
          posts.slice(0, 6).map((post) => (
            <div className="border p-5 rounded-xl" key={post._id}>
      ...
      <div className="flex justify-center my-10">
        <button
          ...
          onClick={handlePrvBtn}
          className="px-4 py-2 mx-1 text-gray-500 capitalize hover:bg-blue-500 dark:hover:bg-blue-500 hover:text-white transition-all bg-white rounded-md dark:bg-gray-800 dark:text-gray-600"
        >
        ...
        {pages &&
          pages.map((page, inx) => (
            <button
              ...
              key={inx}
              className={`hidden px-4 py-2 mx-1 text-black transition-colors duration-300 transform  rounded-md sm:inline dark:bg-gray-800 dark:text-gray-200 ${
                currentPage == page
                  ? "bg-blue-500 text-white"
                  : "text-black bg-slate-600"
